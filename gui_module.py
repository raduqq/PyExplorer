...
class CloseButton(GameObject):
    ...
    def draw(self):
        pygame.draw.circle(self.game.window, Const.YELLOW, (int(self.position[0]), int(self.position[1])), Const.BUTTON_RADIUS)
        pygame.draw.circle(self.game.window, Const.BLACK, (int(self.position[0]), int(self.position[1])), Const.BUTTON_RADIUS, Const.BORDER_THICKNESS)
...
class File(GameObject):
    def __init__(self, game, position, name):
        ...
        self.icon = pygame.transform.scale(self.icon, (Const.ICON_WIDTH, Const.ICON_HEIGHT))
        self.text = self.game.font.render(self.name, False, Const.BLACK)

    def draw(self):
        self.game.window.blit(self.icon, self.position)
        self.game.window.blit(self.text, (self.position[0] + Const.FILE_TEXT_OFFSET_X, self.position[1] + Const.FILE_TEXT_OFFSET_Y))
...
class Highlighter(GameObject):
    ...
    def draw(self):
        ...
        pygame.draw.rect(self.game.window, Const.BLUE, (self.position[0] + counter * Const.PATH_WIDTH + 2, self.position[1] + 2, Const.PATH_WIDTH - 4, Const.PATH_HEIGHT - 4))
        self.game.window.blit(text, (self.position[0] + Const.PATH_WIDTH // Const.TEXT_TO_RECT_WIDTH_RATIO + counter * Const.PATH_WIDTH, self.position[1] + Const.PATH_HEIGHT // Const.TEXT_TO_RECT_HEIGHT_RATIO))
...
class Filepath(GameObject):
    ...
    def draw(self):
        ...
        pygame.draw.rect(self.game.window, Const.YELLOW, (self.position[0], self.position[1], Const.PATH_WIDTH, Const.PATH_HEIGHT), 0)
        pygame.draw.rect(self.game.window, Const.BLACK, (self.position[0], self.position[1], Const.PATH_WIDTH, Const.PATH_HEIGHT), Const.BORDER_THICKNESS)
        self.game.window.blit(text, (self.position[0] + Const.PATH_WIDTH // Const.TEXT_TO_RECT_WIDTH_RATIO, self.position[1] + Const.PATH_HEIGHT // Const.TEXT_TO_RECT_HEIGHT_RATIO))
...
class FrontButton(GameObject):
    ...
    def draw(self):
        pygame.draw.rect(self.game.window, Const.YELLOW, (self.position[0], self.position[1], Const.NAV_BUTTON_SIZE, Const.NAV_BUTTON_SIZE), 0)
        pygame.draw.rect(self.game.window, Const.BLACK, (self.position[0], self.position[1], Const.NAV_BUTTON_SIZE, Const.NAV_BUTTON_SIZE), Const.BORDER_THICKNESS)
        self.game.window.blit(self.text, (self.position[0] + Const.NAV_BUTTON_SIZE // 3, self.position[1] + Const.NAV_BUTTON_SIZE // 3))
...
class Game:
    def __init__(self, curr_dir):
        ...
        close_butt = CloseButton(self, [Const.SCREEN_WIDTH - Const.BUTTON_RADIUS, Const.BUTTON_RADIUS])
        self.const_objects = [back_butt, close_butt, front_butt]

    # Update the files in the OS Module
    def update_files(self):
        dir_objects = [Directory(self, [Const.FILE_X_START, Const.FILE_Y_START + (i * 30)], name) for i, name in enumerate(self.explorer.get_dir_list())]
        curr_counter = len(dir_objects)
        file_objects = [File(self, [Const.FILE_X_START, Const.FILE_Y_START + ((i + curr_counter) * 30)], name) for i, name in enumerate(self.explorer.get_file_list())]
        filepath = [Filepath(self, (2 * Const.NAV_BUTTON_SIZE + i * Const.PATH_WIDTH, 0), dir) for i, dir in enumerate(self.explorer.get_path_list())]
